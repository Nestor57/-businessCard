...
$('.landPageBlockFoto').css({"animation":'clicked 1.5s'});
...
$('.landPageBlockFoto').animate({left:'0'},800,function(){$('.landPageBlockFoto').css({"position": 'relative'});});

$('#startWords1').animate({left:'calc(50% - 150px)'},800);

  i=1;
var intervalForBlock=setInterval(function(){

$('.startWords').animate({opacity:'1'},800);

$('#startBlock-'+i).animate({opacity:'1',marginTop:'100'},600);

i++;
if (i>6) { clearInterval(intervalForBlock);}
},200);
...
$stateProvider
  .state('landingPage',{
   ...
   .state('basicSkills',{
    url:'/basicSkills',
    templateUrl:'pages/basicSkills.html'
  })
...
if ($('.navbar').css('opacity')==0&&tabSelected!=1){$('.navbar').animate({opacity:'1',top:'0'},600);}
...
$timeout(function () {
      $scope.containerHeight = $('#repeatingForm').height();
    }, 0);  
...
if ($('.navbar').css('opacity')==1){$('.navbar').animate({opacity:'0',top:'0'},600);}
...
if ($('#practic-1').height()+48*2<window.pageYOffset+$(window).height()*0.5&&$('#practic-2').css('opacity')==0){

  $('#practic-2').animate({opacity:'1'},600);

  $('.animatedBounce').animate({top:$('#practic-2').height()+$('#practic-2').offset().top+48*2},600);

 // alert($('#practic-2').height()+$('#practic-2').offset().top);
}

if ($('#practic-1').height()+$('#practic-2').height()+48*3<window.pageYOffset+$(window).height()*0.5&&$('#practic-3').css('opacity')==0){

  $('#practic-3').animate({opacity:'1'},600);

  $('.animatedBounce').animate({top:$('#practic-3').height()+$('#practic-3').offset().top+48*3},600);
}

if ($('#practic-1').height()+$('#practic-2').height()+$('#practic-3').height()+48*4<window.pageYOffset+$(window).height()*0.5&&$('#practic-4').css('opacity')==0){

  $('#practic-4').animate({opacity:'1'},600);

    $('.animatedBounce').animate({top:$('#practic-4').height()+$('#practic-4').offset().top+48*4},600);
}

if ($('#practic-1').height()+$('#practic-2').height()+$('#practic-3').height()+
  $('#practic-4').height()+48*5<window.pageYOffset+$(window).height()*0.5 &&$('#practic-5').css('opacity')==0){

  $('#practic-5').animate({opacity:'1'},600);
}

if ($('#practic-1').height()+$('#practic-2').height()+$('#practic-3').height()+
  $('#practic-4').height()+$('#practic-5').height()+48*6<window.pageYOffset+$(window).height()*0.5 &&$('#practic-6').css('opacity')==0){

  $('#practic-6').animate({opacity:'1'},600);
}

if ($('#practic-1').height()+$('#practic-2').height()+$('#practic-3').height()+
  $('#practic-4').height()+$('#practic-5').height()+$('#practic-6').height()+48*7<window.pageYOffset+$(window).height()*0.5&&$('#practic-7').css('opacity')==0){

  $('#practic-7').animate({opacity:'1'},600);
}
}
    };
...
var practic = [{
      name: 'Развиваться друг от друга, учиться друг от друга, созидать друг от друга.',
    text:"",
      images: [
        "image1.jpg",
        "image2.jpg"
...
var comments=[
 ...
 "«Меня не понимает мой руководитель. У меня не получается объяснить ему, почему у меня до сих не укомплектован штат отдела»",
 "«Мы проводим регулярно совещания каждую неделю. Но рабочие вопросы быстрее не решаются. Одна тема может обсуждаться неделями»",
 "«Чем занимаются мои подчиненные? Предложений по изменениям много, а толкового нет!»",
 "«И зарплата достойная, и повышение скоро, а на работу идти не хочется. В офисе даже обедаем молча»",
 "«Наш проект начинался с общего выработанного плана. Прошло только две недели, а мы уже как лебедь, рак и щука»",
 "«Я вполне доброжелательно и конструктивно даю своим подчиненным обратную связь. От них ожидаю положительных изменений в результатах работы, а получаю обиды...»",
 "«Мне не всегда сразу удается понять, что хочет от меня мой руководитель. А переспрашивать как-то неловко,отсюда частые ошибки при выполнении работы»",
 "«Все решения,договоренности по срокам и планам работы мы после обсуждения принимаем в подразделениях вместе со всеми руководителями. Но потом недели уходят на электронную переписку,телефонные звонки и дополнительные встречи перед тем, как начать работу над новым проектом!»"

];

var startWords=[

 "Диалог - это не средство",
"Диалог - это цель",
"Диалог - это не преддверие к действию",
"Диалог - это действие",
"Диалог - это не средствораскрытия характера человека",
"В диалоге человек становится тем, кто он есть для себя и для других",
"«Быть - значит общаться диалогически» М.М. Бахтин",
...
"Быть понятым",
"Понять другого",
"Всегда уметь договариваться",
"С позитивом выйти из конфликта",
"Не создавать конфликт",
"Находить взаимовыгодные решения",
"Генерировать идеи",
"Увидеть горизонты своего развития"
];